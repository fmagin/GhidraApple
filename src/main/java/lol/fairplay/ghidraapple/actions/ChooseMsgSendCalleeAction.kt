...
class ChooseMsgSendCalleeAction: AbstractDecompilerAction("Choose msgSend Callee") {
    ...
    init {
        description = ""
        popupMenuData = MenuData(arrayOf("Choose Selector Callee"), GhidraApplePluginPackage.PKG_NAME)
    }
    ...
    override fun isEnabledForDecompilerContext(ctx: DecompilerActionContext): Boolean {
        ...
        if (func.name == "objc_msgSend") {
            return true
        }
        for (tag in func.tags) {
            if (tag.name == SelectorTrampolineAnalyzer.TRAMPOLINE_TAG) {
                return true
            }
        }
    return false
